...
export default class AddRepositoryDialogController {
    ...
    onAddRepositorySuccess_() {
        ...
        /** @type {string} @desc repository 添加成功 */
        let MSG_repository_success_title = goog.getMsg('添加成功');
        this.toastr["warning"](MSG_repository_success_title, 0, {
            closeButton: true,
            timeOut: 10000,
        });
        this.mdDialog_.hide();
    ...
    onAddRepositoryError_(err) {
        this.log_.error(err);
        /** @type {string} @desc repository 添加失败 */
        let MSG_repository_error_title = goog.getMsg('添加失败');
        this.toastr["warning"](MSG_repository_error_title, 0, {
            closeButton: true,
            timeOut: 10000,
        });
        this.mdDialog_.hide();
...
function i18n() {
    return {
        ...
        /** @export {string} @desc User help for the pod count update dialog (on the repository
            list page). */
        MSG_REPO_LIST_ADD_REPOSITORY_USER_HELP: goog.getMsg('Specify the name and URL of a chart repository to add'),
        ...
        /** @export {string} @desc  只能输入字母数字下划线 Only input the alphanumeric underline*/
        MSG_REPO_LIST_REPOSITORY_NAME_PATTERN_WARNING: goog.getMsg('只能输入字母数字下划线'),
    };
}