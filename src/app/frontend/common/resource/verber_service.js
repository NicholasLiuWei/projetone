    ...
    export class VerberService {
        ...
        deleteErrorCallback(err) {
            if (err) {
                /** @type {string} @desc resourcecard 删除失败提示 */
                let MSG_resourcecard_delete_button = goog.getMsg('确定');
                /** @type {string} @desc resourcecard 删除失败提示 */
                let MSG_resourcecard_delete_title = goog.getMsg('服务器错误');
                /** @type {string} @desc resourcecard 删除失败提示 */
                let MSG_resourcecard_delete_con = goog.getMsg('若多次删除失败且30分钟未自动恢复，请联系厂商解决');
                this.mdDialog_.show(this.mdDialog_.alert()
                    .ok(MSG_resourcecard_delete_button)
                    .title(MSG_resourcecard_delete_title)
                    .textContent(MSG_resourcecard_delete_con));
            }
        }
