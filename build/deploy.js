 ...
 */
gulp.task('docker-image:head', ['build', 'docker-file', 'app-file'], function() {
    return buildDockerImage([
        [conf.deploy.headImageName, conf.paths.dist]
    ]);
});
...
/**
 * Processes the appmananger and monitor file and places it in the dist folder for building.
 */
gulp.task('app-file', ['clean-dist'], function() {
    return appFile(conf.paths.dist);
});
 ...
 * @return {stream}
 */
function appFile(outputDirs, doneFn) {
    return gulp.src(path.join(conf.paths.deploySrc, '/app/*'))
        .pipe(multiDest(outputDirs, doneFn));
}